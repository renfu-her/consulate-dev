...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td class="form_list_2">
           <select name="CID1" dataType="Require" msg="菜單分類未輸入">		  
		  <?php	
	$rs_class = $db->Execute("SELECT * FROM #@#class order by SortOrder asc");	
	while($row_class = $rs_class->FetchRow()){	
	?>
	<option value="<?php echo $row_class['ID'];?>" <? if($row['CID1']==$row_class['ID']){echo "selected";}?>><?php echo $row_class['className'];?></option>
	 <?php }?> 
	 </select>
        </td>
      ...
      <?php }?>
        <tr>
          ...
          <td width="63%" class="form_list_2"><label>
            <input name="pro_name" type="text" id="ProName" size="50" dataType="Require" msg="產品名稱未輸入" value="<?=$row['pro_name']?>"/>
          </label></td>
        ...
        <tr>
          ...
          <td class="form_list_2"><input name="price" type="text" id="price" size="10" value="<?=$row['price']?>" dataType="Integer" msg="價錢請輸入整數"/></td>
          <td class="form_list_3">&nbsp;</td>
        </tr>
        <?php if($row['CID']!=1){?>
        <?php }?>
