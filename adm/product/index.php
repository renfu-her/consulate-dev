...
<body>
<div id="title_menu"><?php echo $admin_lang['product_management'];?> &gt; <?php echo $admin_lang['product_management'];?>列表</div>
<div id="menu_bar">
  <form action="" method="get" enctype="application/x-www-form-urlencoded" name="form1" id="form1" style="width:70%;float:left;text-align:left;">
	...
	?>
	<option value="<?php echo $row_class['ID'];?>" <? if($catid==$row_class['ID']){echo "selected";}?>><?php echo $row_class['className'];?></option>
	 <?php }?> 
	 </select>
 ...
 <div id="main">
   <table width="100%" border="0" cellspacing="1" cellpadding="1">
     <tr class="list_menu">
       ...
       <td width="6%">售價</td>
       <td width="5%">排序</td>
       <td width="8%" align="center">是否上架</td>
	...
	$rs_CID = $db->Execute("SELECT * FROM #@#food order by ID asc");	
	...
	if($catid!=""){
	 ...
	 $psql="SELECT * from #@#product $where AND CID<>6 order by SortOrder asc,pro_id asc,CID desc";
	 $Pager->Init($psql);
	 while($row = $Pager->FetchRow()){
		
	 ?>
     <tr <?php echo $Pager->ListClass();?>>
       <td class="line"><?php echo $CID_array[$row['CID']];?></td>
       <td class="line"><?php echo keyfont($row['pro_name']);?></td>
       <td align="center"class="line"><?php echo ($row['command']=='Y'?"<font color='#0000FF'>是</font>":"否");?></td>
       ...
       <td width="4%" align="center" class="line"><a href="edit.php?ID=<?php echo $row['pro_id'];?>&catid=<?=$catid?>&Page=<?=$Page?>"><img src="../images/icon_edit.gif" alt="編輯" width="18" height="17" border="0" /></a>
	    	   </td>
